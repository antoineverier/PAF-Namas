...
channel_names = ['Red', 'Green', 'Blue']  # Names for color channels

for channel1, channel2 in combinations:
    histogram, x_edges, y_edges = np.histogram2d(
        image[:, :, channel1].ravel(),
        image[:, :, channel2].ravel(),
        bins=bins,
        range=[(0, 255), (0, 255)]
    )
...
for i, (channel1, channel2) in enumerate(combinations):
    axs[i].imshow(histograms[i], cmap='hot', origin='lower', extent=[0, 255, 0, 255])
...
plt.suptitle("Color Histogram (RGB)")
plt.tight_layout()
plt.show()